...
function PRMBeginRequest() {
	...
	if (postBackElementFocus != null && postBackElementFocus.className != "hidden") {
...
function PRMEndRequest(sender, args) {
	...
	if (HideHourglassAndShowMessagePosponed)
		HideHourglassAndShowMessage();
}
...
function DisplayCharts() {
	...
	for (i = 0; i <= $Feedback.length - 1; i++) {
		...
		var sHtml = "";
		var sSqTpl = "<rect x='__x' y='2' rx='2' ry='2' width='__width' height='" + nSize + "' fill='__color' stroke='__color' stroke-width='2' />";
		var sLnTpl = "<line x1='__x1' y1='1' x2='__x2' y2='__y2' fill='__color' stroke='__color' stroke-width='2' />"

		sHtml = "<svg width='" + String(nMaxValue * (nSize + 4)) + "' height='" + String(nSize + 3) + "'>"
		...
		for (j = 0; j <= nMaxValue - 1; j++) {
			...
			if (j < nValue) {
				...
				nFillRatio = nFillRatio > .90 ? 1 : (nFillRatio < .10 ? 10 : nFillRatio);
...
function panelExpandCollapseAll(sExpandCollapse) {
	$('[role="tablist"]').each(function () {
		...
		if (sExpandCollapse == "expand")
			PanelCollapse($(this).attr('id'), "show");

		if (sExpandCollapse == "collapse")
			PanelCollapse($(this).attr('id'), "hide");
	});

	if (sExpandCollapse == "expand")
		SaveState("expandAll", "false");

	if (sExpandCollapse == "collapse")
		SaveState("collapseAll", "false");
}
...
function GetPageAccordionState(CompanyID, UserRef, PageName) {
	...
	$.ajax({
		url: '../Common/CfgSaveState.aspx?CompanyID=' + CompanyID + '&UserRef=' + UserRef + '&Page=' + PageName,
		type: 'get',
		dataType: 'text',
		async: false,
...
function GetAccordionState(accordionState, accordion) {
	...
	if (accordionState.indexOf("{expandAll}1{/expandAll}") >= 0 || accordionState.indexOf("{" + accordion + "}1{/" + accordion + "}") >= 0)
		return "show";
	else
		return "hide";
}
...
(function ($) {
	$.fn.openDialog = function () {
		...
		var dialogCss = ($(this).attr('dialog-additional-css') == undefined ? "dialog-md" : $(this).attr('dialog-additional-css')) + " ui-dialog-full-height";
		var containerCss = $(this).attr('dialog-container-css') == undefined ? "dialog-container" : $(this).attr('dialog-container-css');
		var marginTop = $(this).attr('dialog-margin-top') == undefined ? "0" : $(this).attr('dialog-margin-top');
		var marginBtm = $(this).attr('dialog-margin-bottom') == undefined ? "0" : $(this).attr('dialog-margin-bottom');
		var popupUrl = $(this).attr("dialog-url");
		var displayElem = $(this).attr("dialog-display-element") == undefined ? "" : $(this).attr('dialog-display-element');
		var timer = null;
		...
		var $popup = $("<div class='popup-dialog'></div>")
			...
			.dialog({
				dialogUrl: (popupUrl != undefined ? popupUrl + (popupUrl.indexOf("&Dialog=") == -1 ? "&Dialog=1" : "") : ""),
				...
				open: function (event, ui) {
					...
					$("body").css({
						...
						"overflow-y": "hidden",
						"padding-right": ($(document).height() > $(window).height() ? "16px" : "0")
					});
					...
					var dialogContainerCss = $(this).dialog("option", "dialogContainerCss");

					if (dialogUrl == null || dialogUrl == "") {
						$(this).html((dialogContainerCss == null ? "" : "<div class=\"" + dialogContainerCss + "\">") + $(dialogElement).html() + (dialogContainerCss == null ? "" : "</div>"));
						...
						$("div[role='dialog']").each(function () {
							...
							dialog.css({
								...
								"overflow-y": "scroll",
								"overflow-x": "hidden",
								"height": ($(window).height() - Number(marginTop) - Number(marginBtm)).toString() + "px"
							});
						...
						loading += "<div id=\"divDlgWait\" style=\"display: inline-block; position: fixed; top: 0; bottom: 0; left: 0; right: 0; width: 70px; height: 70px; margin: auto; text-align: center !important;\">";
						loading += "<img alt=\"\" src=\"../images/loading.gif\" style=\" height: 64px; width: 64px;\" />";
						loading += "</div>";

						$(this).html((dialogContainerCss == null ? "" : loading + "<iframe style=\"width: 100%; display: block; overflow-y: hidden; height: 9999999px; \" src=\"" + dialogUrl + "\"></iframe>"));
						...
						$('iframe').on('load', function () {
							$("div[role='dialog']").each(function () {
								...
								if (iframe.contents().find(".dialog-container").height() != null)
									dialog.find(".ui-dialog-content").css("height", iframe.contents().find(".dialog-container").height() + "px");
								else
									dialog.find(".ui-dialog-content").css("height", iframe.contents().find("#divSiteMain").height() + "px");
					...
					$popup.dialog("option", "height", window.parent.$("#divSiteMain").height() - scrollTop);
					window.scrollTo(0, 0);
				...
				close: function (event, ui) {
					...
					if (newDialogId != undefined && newDialogId != null && newDialogId != "") {
			...
			$("div[role='dialog']").each(function () {
				...
				if (displayElem != "") {
					...
					if (iframe.contents().find(".dialog-container").height() != null) {
						...
						if (iframe.contents().find(".dialog-container").height())
							dialog.find(".ui-dialog-content").css("height", iframe.contents().find("#divSiteMain").height() + "px");
						else
							dialog.find(".ui-dialog-content").css("height", window.parent.$("#divSiteMain").height() - scrollTop + "px");
					}
				}
			});
		});
		...
		timer = setInterval(function () {
			$("div[role='dialog']").each(function () {
				...
				if (displayElem != "") {
					...
					if (marginBtm != "0")
						dialog.removeClass("ui-dialog-full-height");
				}
			});
		}, 500);
...
function dialogDeleteConfirm(dialogText, okFunc, cancelFunc, dialogTitle) {
	$('<div class="confirm-dialog-container delete-dialog"><div class="confirm-dialog-title">' + dialogTitle + '</div><div class="confirm-dialog-content">' + dialogText + '</div></div>').dialog({
		draggable: false,
		modal: true,
		resizable: false,
		width: 'auto',
...
function dialogMessage(dialogText, okFunc, dialogTitle) {
	$('<div class="confirm-dialog-container"><div class="confirm-dialog-title">' + dialogTitle + '</div><div class="confirm-dialog-content">' + dialogText + '</div></div>').dialog({
		draggable: false,
		modal: true,
		resizable: false,
		width: 'auto',
...
function InitComponents() {
	$(":input").inputmask();
	$('.date').not(".custom-format").datetimepicker({ format: 'MMM DD, YYYY', useCurrent: false, keyBinds: 't:' });
	$('.time').not(".custom-format").datetimepicker({ format: 'LT', useCurrent: false, keyBinds: 't:' });
	$('.datetime').not(".custom-format").datetimepicker({ format: 'MMM DD, YYYY LT', useCurrent: false, keyBinds: 't:' });
	...
	$('.slick-single-item').each(function () {
		if (!$(this).hasClass('slick-initialized')) {
			$(this).slick({
				prevArrow: '<a class="slick-carousel-control left"><span class="glyphicon glyphicon-chevron-left"></span></a>',
				nextArrow: '<a class="slick-carousel-control right"><span class="glyphicon glyphicon-chevron-right"></span></a>',
				dots: true,
				slidesToShow: 1,
				focusOnSelect: true,
...
function InitDataTable(GridViewID, SearchFieldID, Responsive, Paging, Message) {
	...
	if (Message != undefined && Message != null)
		$('#' + GridViewID + "_paginate").after("<div style='float: left; padding-top: 8px; padding-left: 10px;'>" + Message + "</div>");

	$('#' + GridViewID).removeClass("hidden-onload");
}

function InitDataTableWithGrouping(GridViewID, SearchFieldID, Responsive, Paging, Message, GroupColumnIndex) {
    var table = $('#' + GridViewID).DataTable({
        columnDefs: [{
                "targets": "datatable-nosort",
                "orderable": false
    ...
    if (GroupColumnIndex != undefined) {
        ...
        if (table.column(GroupColumnIndex).data().unique().length > 1)
...
function ModalPopupOnShow(modalPopupClientID) {
	$("#divSiteTopNav").css({
		"overflow-y": "hidden",
		"padding-right": ($(document).height() > $(window).height() ? (scrollBarWidth + 1).toString() + "px" : "0"),
		"z-index": "1"
	});

	$("body").css({
		"overflow-y": "hidden",
		"padding-right": ($(document).height() > $(window).height() ? scrollBarWidth.toString() + "px" : "0")
	});

	modalPopupElement = modalPopupClientID == null || modalPopupClientID == undefined ? $(".modal-popup") : $("#" + modalPopupClientID);
...
function ResizeModalPopup() {
	if (modalPopupElement != null && modalPopupElement != undefined) {
		var modalPopupHeight = modalPopupElement.find(".dialog-container").height() + 30;
		...
		if ($(window).height() > modalPopupHeight) {
			...
			modalPopupElement.css({
				...
				"-webkit-transition": "top " + modalPopupMoveTransition + ", height " + modalPopupMoveTransition,
				"transition": "top " + modalPopupMoveTransition + ", height " + modalPopupMoveTransition
			});
		}
		else {
			modalPopupElement.css({
		...
		modalPopupMoveTransition = modalPopupMoveTransition == "0s" ? "0.2s" : modalPopupMoveTransition;
	}
}
...
function HideHourglassAndShowMessage() {
	...
	if (PRMRequestStatus == "PRMBeginRequest" && HideHourglassAndShowMessageExecuted == true) {
	...
	$(".alert-danger:not('.no-toastr'), .alert-warning:not('.no-toastr')").each(function () {
		if ($(this).html() != "") {
