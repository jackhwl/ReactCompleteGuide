...
;(function($) {
  ...
  var App = {
    ...
    CssClasses: {
      ...
      NOTIFICATION_CLOSE: "notify-container-close>.notify-close>a"
    },
    ...
    init: function init() {
      ...
      this.$notificationClose = $("." + this.CssClasses.NOTIFICATION_CLOSE)
      this.mediaQueryListMobile = window.matchMedia("(max-width: 767px)")
      this.mediaQueryListTablet = window.matchMedia(
    ...
    bindEvents: function bindEvents() {
      this.$leftSidebarCollapse.on(
        "click",
        this.handleLeftSidebarCollapse.bind(this)
      )
      this.$leftSidebarToggle.on(
        "click",
        this.handleLeftSidebarToggle.bind(this)
      )
      this.$overlay.on("click", this.handleLeftSidebarToggle.bind(this))
      this.$rightSidebarToggle.on(
        "click",
        this.handleRightSidebarToggle.bind(this)
      )
      this.$document.on("mouseup", this.handleOutsideClick.bind(this))
      this.$notificationClose.on("click", this.closeNotificationbar)
      this.mediaQueryListMobile.addListener(
        this.handleMediaQueryChangeMobile.bind(this)
      )
      this.mediaQueryListTablet.addListener(
        this.handleMediaQueryChangeTablet.bind(this)
      )
      this.mediaQueryListDesktop.addListener(
        this.handleMediaQueryChangeDesktop.bind(this)
      )
    },
    ...
    closeNotificationbar: function() {
      $(".notify-container-close").addClass("notify-hidden")
      $(".notify-container").addClass("notify-hidden")
      //CreateCookie("NotificationState", "1", 1)
    },
    ...
    handleOutsideClick: function handleOutsideClick(evt) {
      var container = $(
        "." +
          this.CssClasses.RIGHT_SIDEBAR +
          ", ." +
          this.CssClasses.RIGHT_SIDEBAR_TOGGLE
      )
    ...
    handleLeftSidebarCollapse: function handleLeftSidebarCollapse(evt) {
      if (
        !this.$layout.hasClass(this.CssClasses.LAYOUT_LEFT_SIDEBAR_COLLAPSED)
      ) {
    ...
    setNotificationState: function() {
      var notificationState = "" //ReadCookie("NotificationState")
      if (!notificationState) {
        if (!$(".notify-container-close").hasClass("hidden")) {
          if ($(".notify-container-close").hasClass("notify-hidden")) {
            $(".notify-container-close").removeClass("notify-hidden")
            $(".notify-container").removeClass("notify-hidden")
          }
        }
      }
    },
