...
class Header extends React.Component {
    ...
    render() {
        return <div style={{border: `3px solid ${this.context.color}`}}>Header<Title /><TitleF /></div>
    }
}
function TitleF(props) {
    return (
        <ThemeContext.Consumer>
            {
                (value) => (
                    <div style={{border: `3px solid ${value.color}`}}>Function Title</div>
                )
            }
        </ThemeContext.Consumer>
    )
